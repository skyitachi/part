...
static void *bdwgc_malloc(const size_t size)
{
//  (void)ctx;
  return GC_malloc(size);
}

static void *bdwgc_realloc(void *ptr, const size_t new_size)
{
...
static void *dlmalloc_malloc(const size_t size)
{
  return dlmalloc(size);
}

static void *dlmalloc_realloc(void *ptr, const size_t new_size)
{
...
int main(int argc, char **argv)
{
  ...
  int *indexes = (int *)malloc(sizeof(int) * j);
...
//    goto exit;
  ...
  struct ProgressBar pb = {.count = 0, .total = static_cast<int>(j)};
  for (size_t i = 0; i < j; i++) {
    if (!quiet)
      progress_print(&pb);
    int k = indexes[i];
