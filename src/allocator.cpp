...
data_ptr_t Allocator::AllocateData(idx_t size) {
  ...
  if (size >= MAXIMUM_ALLOC_SIZE) {
    ...
    throw std::invalid_argument(
        fmt::format("Requested allocation size of {} is out of range - maximum "
                    "allocation size is {}",
...
data_ptr_t Allocator::ReallocateData(data_ptr_t pointer, idx_t old_size, idx_t size) {
  ...
  if (size >= MAXIMUM_ALLOC_SIZE) {
    assert(false);
    throw std::invalid_argument(
        fmt::format("Requested re-allocation size of {} is out of range - "
...
#endif
  //! Verify that there is no outstanding memory still associated with the
  //! batched allocator Only works for access to the batched allocator through
  //! the batched allocator interface If this assertion triggers, enable
  //! DUCKDB_DEBUG_ALLOCATION for more information about the allocations
