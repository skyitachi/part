...
#ifndef PART_VALIDITY_MASK_H
#define PART_VALIDITY_MASK_H
#include "types.h"
...
public:
  ...
  inline void SetInvalid(idx_t row_idx) {
    if (!validity_mask) {
      assert(row_idx <= STANDARD_VECTOR_SIZE);
...
public:
  ...
  inline void Initialize(idx_t count = STANDARD_VECTOR_SIZE) {
    validity_data = std::make_shared<ValidityBuffer>(count);
...
public:
  ...
  std::string ToString(idx_t count) const;
